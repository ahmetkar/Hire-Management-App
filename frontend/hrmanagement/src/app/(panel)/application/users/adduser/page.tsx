  ...
  const [stage,setStage] = useState("onlyuser")
   const [universities, setUniversities] = useState<University[]>([]);
    ...
    const router = useRouter()
     ...
     const {register:registerUser,handleSubmit:handleSubmitUser,getValues:getValuesUser,setValue:setValueUser,formState:{errors:errorsUser}} = useForm<UserFormData>({
      mode:"onChange"
     });

     const {register:registerStaff,handleSubmit:handleSubmitStaff,setValue:setValueStaff,formState:{errors:errorsStaff}} = useForm<StaffFormData>({
      mode:"onChange"
     });

     const {register:registerUserAndStaff,getValues:getValuesUserAndStaff,handleSubmit:handleSubmitUserAndStaff,setValue:setValueUserAndStaff,formState:{errors:errorsUserAndStaff}} = useForm<UserAndStaffFormData>({
      mode:"onChange"
     });
                ...
                {(stage == "onlyuser") ? (
                  ...
                  <div className='form-group col-md-4'>
                    <label htmlFor="custom-zip">Posta kodu</label>
                            ...
                            {...registerStaff("zipcode",{required: "Posta kodu gereklidir."})}
                            />
                            <div className="invalid-feedback"> 
                             {errorsStaff.zipcode && (
                        <p className='text-red-500 text-sm'>{String(errorsStaff.zipcode.message)}</p>
                    )
                    }
                  </div>
                  </div>
                  ...
                  <div className='form-group col-md-4'>
                    <label htmlFor="custom-zip">Posta kodu</label>
                            <input className="form-control input-zip" id="custom-zip" autoComplete="false" maxLength={9} 
                            {...registerUserAndStaff("zipcode",{required: "Posta kodu gereklidir."})}
                            />
                            <div className="invalid-feedback"> 
                             {errorsUserAndStaff.zipcode && (
                        <p className='text-red-500 text-sm'>{String(errorsUserAndStaff.zipcode.message)}</p>
                    )
                    }
                  </div>
                  </div>
